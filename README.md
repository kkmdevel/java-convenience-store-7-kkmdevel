...
### ⚙️ 재고 초기화 기능
- **설명**: 초기 재고 정보를 `enum`과 products.md를 이용하여 각 재고 객체를 생성합니다.
...
### ⚙️ 주문 검증 및 생성 기능
- **설명**: 입력된 상품명과 수량이 올바른지 검증하여 주문을 진행합니다. 올바른 주문이라면 주문한 상품들을 객체로 생성합니다.

### ⚙️ 프로모션 적용 기능
- **설명**: 특정 상품에 대해 프로모션 혜택을 적용합니다. N+1 형태의 프로모션을 제공하여 프로모션 결과 객체를 생성합니다.
- **요구사항**:
  - 프로모션 혜택을 받기 위한 추가 수량 안내 및 부족 시 일반 재고를 사용하도록 처리합니다.
  - 프로모션 재고 부족 시 혜택을 받지 못하는 수량에 대해 안내합니다.
  - 프로모션 재고 부족 시 혜택을 받지 못하는 수량을 계산에서 제외시킬 시 수량이 0이 된다면 구매 목록에서 제외합니다.
...
### ⚙️ 재고 관리 기능
- **설명**: 구매한 상품에 대한 재고를 차감하고 최신 재고 상태를 유지합니다.
...
│   ├── java/store
│   │   ├── controller
│   │   │   ├── DiscountAndPaymentController.java    // 할인 및 결제 과정을 담당하는 컨트롤러
│   │   │   ├── OrderController.java                 // 주문을 관리하는 컨트롤러
│   │   │   ├── ProductController.java               // 상품 관련 작업을 제어하는 컨트롤러
│   │   │   ├── PromotionController.java             // 프로모션 적용 및 관리를 담당하는 컨트롤러
│   │   │   └── StoreController.java                 // 전체적인 상점 로직을 조율하는 컨트롤러
│   │   ├── domain
│   │   │   ├── enums
│   │   │   │   ├── ProductLines.java                // 상품들의 파일안의 위치를 나타내는 열거형 클래스
│   │   │   │   └── PromotionLines.java              // 프로모션들의 파일안의 위치를 나타내는 열거형 클래스
│   │   │   ├── OrderItem.java                       // 개별 주문 항목을 나타내는 클래스
│   │   │   ├── OrderItemManager.java                // 주문 항목 컬렉션을 관리하는 클래스
│   │   │   ├── Product.java                         // 상품을 나타내는 클래스
│   │   │   ├── ProductManager.java                  // 상품 컬렉션을 관리하는 클래스
│   │   │   ├── Promotion.java                       // 프로모션을 정의하는 클래스
│   │   │   ├── PromotionManager.java                // 프로모션 컬렉션을 관리하는 클래스
│   │   │   └── PromotionResult.java                 // 적용된 프로모션 결과를 저장하는 클래스
│   │   ├── exception
│   │   │   └── ExceptionMessage.java                // 일관된 에러 메시지를 위한 중앙 관리 클래스
│   │   ├── service
│   │   │   ├── MembershipDiscountService.java       // 멤버십 할인 계산을 담당하는 서비스
│   │   │   ├── ProductManagementService.java        // 상품 작업을 관리하는 서비스
│   │   │   ├── PromotionDiscountService.java        // 프로모션 할인 계산을 담당하는 서비스
│   │   │   └── TotalPriceCalculator.java            // 할인 적용 후 총 금액을 계산하는 서비스
│   │   ├── utils
│   │   │   ├── DataReader.java                      // 파일에서 데이터를 읽는 유틸리티 클래스
│   │   │   └── Parser.java                          // 사용자 입력을 파싱하고 검증하는 유틸리티 클래스
│   │   ├── view
│   │   │   ├── InputView.java                       // 사용자 입력을 처리하는 클래스
│   │   │   ├── OutputMessage.java                   // 출력 메시지를 정의한 상수 클래스
│   │   │   └── OutputView.java                      // 사용자에게 출력하는 클래스
│   │   └── Application.java                         // 프로그램의 시작점이 되는 메인 클래스
│   ├── resources
│   │   ├── products.md                              // 상품 목록과 상세 정보를 담고 있는 파일
│   │   └── promotions.md                            // 프로모션 목록과 상세 정보를 담고 있는 파일
```
---